...
accThreshold = [0.75, 0.55];      % start by thresholding to 50% and 90% seen (75% and 95% 2afc accuracy, respectively)
    nThresh = length(accThreshold); 
showInstructions = 0;           % whether or not to show instructions (0 no, 1 yes) 
...
jitterSpace = ppd;        % distance in pixels between most extreme stim positions in each dimension (x, y) 
%  Noise
...
transpNoise = 127;          % 
dimNoise = 12 * ppd;        %
freqNoise = 2;              % pixel size of noise
refreshNoise = 0.033;           % seconds to wait before refreshing noise
% lastRefreshTime = GetTime(); %Initialize last refresh time
...
% Init Stim Timing and Location %         
        bufferFrames = round(bufferTime/refreshNoise); jitterFrames = round(jitterTime/refreshNoise); stimFrames = round(stimTime/refreshNoise);
            ...
            while  f < (bufferFrames + FrameJitterPerTrial(trial) + stimFrames-1)
                ...
                f = f+1;
                    ...
                    Screen('DrawTexture', mainWindow, gaborTex, [], CenterRectOnPoint(stimRect, XJitterPerTrial(trial), YJitterPerTrial(trial)),...
                        orientGabor, [], [], [], [], kPsychDontDoRotation, gaborProperties);
                    %%%little sqaure for timing test code here!
                    oldNoiseTex = noiseTex;
                    noiseTex = readyNoiseFramePTB(mainWindow, dimNoise, transpNoise,freqNoise);  
                    Screen('Flip', mainWindow);
